		...
		{	InputStream input = new FileInputStream(new File(filename));
			...
			{				if(line.startsWith("v "))
				{	for(String s : line.substring(2).trim().split("\\s+"))
					{	if (!s.isEmpty()) vertVals.add(Float.valueOf(s));
				}	}
				else if(line.startsWith("vt"))
				{	String[] parts = line.substring(3).trim().split("\\s+");
					if (parts.length >= 2) {
						stVals.add(Float.valueOf(parts[0]));
						stVals.add(Float.valueOf(parts[1]));
					}
				}
				else if(line.startsWith("vn"))
				{	for(String s : line.substring(2).trim().split("\\s+"))
					{	if (!s.isEmpty()) normVals.add(Float.valueOf(s));
				}	}
			else if(line.startsWith("f"))
				{	String[] tokens = line.substring(2).trim().split("\\s+");
					int[] indices = (tokens.length == 4) ? new int[] { 0, 1, 2, 0, 2, 3 } : (tokens.length == 3) ? new int[] { 0, 1, 2 } : null;
					if (indices == null) continue;
					for (int i = 0; i < indices.length; i++)
					{	String s = tokens[indices[i]];
						String v = s.split("/")[0];
						String vt = s.split("/")[1];
						String vn = s.split("/")[2];
